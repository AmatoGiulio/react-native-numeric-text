...
internal class NumericRollEngine {
  ...
  data class GlyphSample(
    ...
    val kind: TokenKind,
    val role: GlyphRole,
    /** Unique identity of a live STACK transition; 0 for non-STACK samples. */
    val renderId: Int = 0,
    val x: Float,
    val offsetY: Float,
    val alpha: Float,
  ...
  companion object {
    ...
    const val STACK_EXIT_BLUR_SPEEDUP = 1.35f
    /**
    ...
    const val STACK_CROWD_SHORTEN = 0.0f
    
    const val STACK_ARRIVAL_SHARPNESS = 2.5f

    const val STACK_ARRIVAL_GATE = 0.0f

    const val STACK_ALPHA_FLOOR = 0.0f

    const val STACK_CROWD_SPEEDUP = 0.70f
  ...
  fun step(dtSeconds: Float): Boolean {
    ...
    while (iterator.hasNext()) {
      ...
      if (column.pending.isNotEmpty()) {
        ...
        if (
          column.pending.isNotEmpty() &&
          column.pending.first().dueAtNanos <= nowNanos
        ) {
...
// La stack conserva intenzionalmente gli ultimi due Entry per evitare
// buchi durante un handoff. Un Entry sotto la soglia di rendering,
// però, non deve impedire al glifo finale di diventare stabile.
val hasOtherVisibleEntry =
  column.entries.indices.any { j ->
    j != i &&
      column.entries[j].alpha > 0.01f
  }
...
val settled =
  ...
  private fun stepEntries(column: Column, response: Float, dt: Float): Boolean {
    ...
    val oneWayCrowd =
      column.crowd * column.crowdRaw

    val reversalSuppression =
      (1f - column.flipRaw).coerceIn(0f, 1f)
    ...
    val fast =
      (
        2.0 * Math.PI /
          max(
            0.05f,
            STACK_RESPONSE_SECONDS * base / geometryRush
          )
      ).toFloat()
    ...
    val slow =
      (
        2.0 * Math.PI /
          max(
            0.05f,
            STACK_SLOW_RESPONSE_SECONDS * base
          )
      ).toFloat()
    ...
    val scale =
      (
        2.0 * Math.PI /
          max(
            0.05f,
            STACK_SLOW_RESPONSE_SECONDS * base / geometryRush
          )
      ).toFloat()
    ...
    val blur =
      (
        2.0 * Math.PI /
          max(
            0.05f,
            STACK_BLUR_RESPONSE_SECONDS * base
          )
      ).toFloat()
...
if (
  abs(bError) > POSITION_EPSILON ||
  abs(entry.bVelocity) > VELOCITY_EPSILON
) {
  entry.bVelocity +=
    (
  ...
  // Il blur è uno stato limitato 0..1.
  // Non conservare velocità "nascosta" oltre i suoi estremi.
  if (
  ...
  entry.bVelocity = 0f
      ...
      if (abs(qError) > POSITION_EPSILON || abs(entry.qVelocity) > VELOCITY_EPSILON) {
        entry.qVelocity +=
          ((scale * scale * qError) -
            (2f * STACK_SLOW_DAMPING * scale * entry.qVelocity)) * dt
